...
const storage = {
  ...
  sfxEnabled: 'diceBatchGridSfxEnabled',
};
...
const sfxSources = {
  erase: 'assets/audio/erace.mp3',
  refill: 'assets/audio/hojyu.mp3',
  joker: 'assets/audio/joker.mp3',
  gameOver: 'assets/audio/last.mp3',
  lock: 'assets/audio/lock.mp3',
  start: 'assets/audio/start.mp3',
};
const sfx = {
  enabled: localStorage.getItem(storage.sfxEnabled) !== 'off',
  volume: 0.45,
  audio: {},
  unlocked: false,
};
function preloadAudio() {
  ...
  for (const [name, src] of Object.entries(sfxSources)) {
    if (sfx.audio[name]) continue;
    const audio = new Audio(src);
    audio.preload = 'auto';
    audio.volume = sfx.volume;
    audio.addEventListener?.('error', () => {});
    sfx.audio[name] = audio;
  }
}
function updateSfxUi() { if (sfxToggleEl) sfxToggleEl.textContent = `SE ${sfx.enabled ? 'ON' : 'OFF'}`; }
function setSfxEnabled(enabled) { sfx.enabled = Boolean(enabled); localStorage.setItem(storage.sfxEnabled, sfx.enabled ? 'on' : 'off'); updateSfxUi(); }
function toggleSfx() { setSfxEnabled(!sfx.enabled); }
function playSound(soundName) {
  if (!sfx.enabled) return;
  preloadAudio();
  const audio = sfx.audio[soundName];
  if (!audio) return;
  try {
    audio.pause?.();
    audio.currentTime = 0;
...
function updateActionButton() { const hasStock = state.trayDice.some(d => !isOnBoard(d.id)); const hasUnlocked = Boolean(state.pendingJoker) || state.board.flat().some(d => d && !d.locked); const isRoll = !hasStock && !hasUnlocked; rollButton.textContent = isRoll ? '補充' : '確定'; if (actionHintEl) actionHintEl.textContent = isRoll ? '補充：サイコロを4つ出します' : '確定：置いたサイコロをロックして判定します'; }
function render() {
  boardEl.innerHTML = '';
  for (let r=0;r<BOARD_SIZE;r++) for (let c=0;c<BOARD_SIZE;c++) { const cell = document.createElement('div'); cell.className = 'cell'; cell.dataset.row = r; cell.dataset.col = c; const pending = state.pendingJoker && state.pendingJoker.row === r && state.pendingJoker.col === c; const die = pending ? state.pendingJoker.die : state.board[r][c]; if (die) { const d = dieElement(die); if ((!die.locked || pending) && !state.animating) d.addEventListener('pointerdown', startDrag); cell.appendChild(d); } boardEl.appendChild(cell); }
  ...
  scoreEl.textContent = formatPoints(state.score); highScoreEl.textContent = formatPoints(state.highScore); modeLabelEl.textContent = GAME_MODES[state.mode].label; countEl.textContent = formatJokerCountdown(state.jokerStock >= MAX_JOKERS ? 'MAX' : (state.jokerChargePoints ?? state.jokerCountdown), state.jokerThreshold); msgEl.textContent = state.message; rollButton.disabled = !state.started || state.animating || state.gameOver; updateActionButton(); renderRankings(); }
function isOnBoard(id) { return state.pendingJoker?.die.id === id || state.board.flat().some(d => d?.id === id); }
function startDrag(event, newDie = null) { if (state.animating || state.gameOver) return; event.preventDefault?.(); event.stopPropagation?.(); state.dragging?.ghost?.remove?.(); const original = event.currentTarget; const id = newDie?.id || original.dataset.id; const die = newDie || findDie(id); const fromPendingJoker = state.pendingJoker?.die.id === id; if (!die || (die.locked && !fromPendingJoker)) return; state.dragging = { id, die, fromJoker: Boolean(newDie), fromPendingJoker, ghost: dieElement(die), pointerId: event.pointerId, source: original }; state.dragging.ghost.classList.add('dragging'); document.body.appendChild(state.dragging.ghost); original.setPointerCapture?.(event.pointerId); moveGhost(event); window.addEventListener('pointermove', moveGhost, { passive: false }); window.addEventListener('pointerup', dropDie, { once: true }); window.addEventListener('pointercancel', cancelDrag, { once: true }); }
function findDie(id) { return state.pendingJoker?.die.id === id ? state.pendingJoker.die : state.trayDice.find(d => d.id === id) || state.board.flat().find(d => d?.id === id); }
function moveGhost(e) { if (!state.dragging) return; e.preventDefault?.(); e.stopPropagation?.(); state.dragging.ghost.style.left = `${e.clientX}px`; state.dragging.ghost.style.top = `${e.clientY}px`; state.dragging.ghost.style.transform = 'translate(-50%, -50%)'; }
...
function cleanupDrag() { window.removeEventListener('pointermove', moveGhost); const drag = state.dragging; drag?.source?.releasePointerCapture?.(drag.pointerId); drag?.ghost?.remove?.(); }
function cancelDrag() { cleanupDrag(); state.dragging = null; render(); }
function dropDie(e) { e.preventDefault?.(); e.stopPropagation?.(); cleanupDrag(); const drag = state.dragging; if (!drag) return; const cell = nearestCell(e.clientX, e.clientY); if (cell) { const r = Number(cell.dataset.row), c = Number(cell.dataset.col); const occupied = state.board[r][c]; const same = occupied?.id === drag.id; if (drag.die.joker && (drag.fromJoker || drag.fromPendingJoker)) { if (!occupied || (occupied.locked && !occupied.joker)) { state.pendingJoker = { die: drag.die, row: r, col: c }; if (drag.fromJoker) state.jokerStock--; state.message = '未固定サイコロは確定で固定されます。'; } } else if (!occupied || same || (drag.die.joker && occupied.locked && !occupied.joker)) { removeFromBoard(drag.id); state.board = placeDie(state.board, r, c, drag.die); state.message = '未固定サイコロは確定で固定されます。'; } } state.dragging = null; render(); }
function removeFromBoard(id) { for (let r=0;r<BOARD_SIZE;r++) for (let c=0;c<BOARD_SIZE;c++) if (state.board[r][c]?.id === id && !state.board[r][c].locked) state.board[r][c] = state.board[r][c].underlyingDie ? { ...state.board[r][c].underlyingDie } : null; }
function commitPendingJoker() { if (!state.pendingJoker) return; const { die, row, col } = state.pendingJoker; const occupied = state.board[row][col]; if (occupied?.joker) { state.pendingJoker = null; state.message = 'ジョーカーの上にジョーカーは置けません。'; return; } state.board = placeDie(state.board, row, col, die); state.pendingJoker = null; }
async function roll() { if (state.animating || state.gameOver) return; commitPendingJoker(); const hasStock = state.trayDice.some(d => !isOnBoard(d.id)); const unlocked = state.board.flat().filter(d => d && !d.locked); if (!hasStock && unlocked.length === 0) { state.trayDice = rollBalancedDice(state.previousBatchDice); playSound('refill'); state.previousBatchDice = state.trayDice.map(d => ({ ...d })); state.message = 'ストックを盤面へドラッグしてください。'; render(); return; } if (unlocked.length === 0) { state.message = 'サイコロを配置してください'; render(); return; } const locked = lockUnlockedDice(state.board); if (locked.locked.length > 0) playSound('lock'); state.board = locked.board; state.trayDice = state.trayDice.filter(d => !locked.locked.some(x => x.id === d.id)); await resolveMatches(locked.locked.map(d => d.id)); if (state.trayDice.length === 0 && !state.gameOver) state.message = 'ストック完了。次は補充で新しい4個を生成します。'; render(); }
...
function startNewGame(mode = state.mode, showStart = false) { state.dragging?.ghost?.remove?.(); closeGameOverModal(); const next = createInitialState(mode); Object.assign(state, next, { rankings: loadRankings(next.mode), highScore: loadHighScore(next.mode), animating: false, dragging: null, pendingJoker: null, rankRegistered: false, started: !showStart }); activeGameOverId = null; playerNameEl.value = ''; startEl.hidden = !showStart; startEl.setAttribute?.('aria-hidden', showStart ? 'false' : 'true'); startEl.style.display = showStart ? '' : 'none'; render(); }
function selectedStartMode() { return document.querySelector('input[name="gameMode"]:checked')?.value || DEFAULT_MODE; }
function startSelectedGame() { unlockBgm(); preloadAudio(); sfx.unlocked = true; playSound('start'); startNewGame(selectedStartMode(), false); }
...
export const __testing = { state, endGame, restartGame, registerRanking, startNewGame, startDrag, dropDie, cancelDrag, commitPendingJoker, playSound, setSfxEnabled, preloadAudio };
