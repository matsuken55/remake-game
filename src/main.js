...
function updateActionButton() { const hasStock = state.trayDice.some(d => !isOnBoard(d.id)); const hasUnlocked = state.board.flat().some(d => d && !d.locked); rollButton.textContent = !hasStock && !hasUnlocked ? '補充' : '確定'; }
function render() {
  boardEl.innerHTML = '';
  ...
  scoreEl.textContent = state.score; highScoreEl.textContent = state.highScore; modeLabelEl.textContent = GAME_MODES[state.mode].label; countEl.textContent = state.jokerStock >= MAX_JOKERS ? 'MAX' : state.jokerCountdown; msgEl.textContent = state.message; rollButton.disabled = !state.started || state.animating || state.gameOver; updateActionButton(); renderRankings(); }
...
function fillRankingList(listEl, rankings = state.rankings) { listEl.innerHTML = ''; rankings.slice(0, 15).forEach((r,i) => { const li = document.createElement('li'); li.textContent = `${i+1}. ${r.name} ${r.score}`; listEl.appendChild(li); }); }
