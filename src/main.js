import { ALL_HANDS, BOARD_SIZE, COLOR_CODES, COLOR_LABELS, createInitialState, createJoker, evaluateBoard, hasEmptyCell, lockUnlockedDice, MAX_JOKERS, placeDie, rollBalancedDice, saveRanking, awardJokers } from './game.js';
...
function fillRankingList(listEl) { listEl.innerHTML = ''; state.rankings.forEach((r,i) => { const li = document.createElement('li'); li.textContent = `${i+1}. ${r.name} ${r.score}`; listEl.appendChild(li); }); }
function renderRankings() { fillRankingList(rankingEl); fillRankingList(rankingViewListEl); }
...
function registerRanking(event) { event.preventDefault(); if (!state.gameOver || overEl.hidden || state.rankRegistered || activeGameOverId === null || registeredGameOverId === activeGameOverId) return; state.rankings = saveRanking(state.rankings, $('#playerName').value, state.score); saveLocalRanking(localStorage, storage.ranks, state.rankings); state.rankRegistered = true; registeredGameOverId = activeGameOverId; resetToNewGame(); }
...
$('#rankingButton').addEventListener('click', () => { renderRankings(); rankingViewEl.showModal(); });
