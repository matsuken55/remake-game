...
function createFakeDocument(){const ids=['#board','#tray','#roll','#score','#highScore','#message','#jokerWindow','#jokerCountdown','#log','#gameOver','#rankingList','#scoreDialog','#rankForm','#playerName','#rules','#closeRules','#restart'];const elements=new Map(ids.map(id=>[id,new FakeElement(id.slice(1)==='rankForm'?'form':'div')]));elements.get('#playerName').value='AAA';return{querySelector:s=>{const e=elements.get(s);if(!e)throw Error(`Missing ${s}`);return e},querySelectorAll:s=>s==='.cell'?elements.get('#board').children:[],createElement:t=>new FakeElement(t),elements};}
