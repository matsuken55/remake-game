...
function formatJokerCountdown(value, threshold = null) { return value === 'MAX' ? value : threshold ? `${Math.max(0, threshold - value)} / ${threshold}点` : formatPoints(value); }
...
function commitPendingJoker() { if (!state.pendingJoker) return; const { die, row, col } = state.pendingJoker; state.board = placeDie(state.board, row, col, die); state.pendingJoker = null; }
